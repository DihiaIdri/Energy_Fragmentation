...
class GradyEnergyCriterion:
    ...
    def plot_energy_unit_volume(self):
        gamma = self.Kavg**2/(2*self.R*self.C0**2)  # Fracture surface energy per unit area
        t = np.linspace(0.01*10**-6, 10**-6, 101)
        fracture_E = 3*gamma/(self.C0*t)
        ...
        E_intercept = 3*gamma/(self.C0*t_intercept)
        print(t_intercept, E_intercept)

        ax = plt.gca()
        ax.set_xlim([t[0], t[100]])
